-- ============================================================
-- ZX Golf App — Sync Function Patch for Training Kit
-- Migration: 021_sync_patch_training_kit.sql
-- Patches sync_upload and sync_download to include:
--   Drill: Target, Description, TargetDistanceUnit, TargetSizeUnit,
--          RequiredEquipment, RecommendedEquipment
--   UserDrillAdoption: HasUnseenUpdate
--   UserTrainingItem: full CRUD
-- ============================================================
  ...
  IF changes ? 'Drill' THEN
    ...
    LOOP
      ...
      INSERT INTO "Drill" (
        ...
        (v_row->>'Target')::DECIMAL,
        v_row->>'Description',
        v_row->>'TargetDistanceUnit',
        v_row->>'TargetSizeUnit',
      ...
      ON CONFLICT ("DrillID") DO UPDATE SET
        ...
        "TargetDistanceUnit" = EXCLUDED."TargetDistanceUnit",
        "TargetSizeUnit" = EXCLUDED."TargetSizeUnit",
        "RequiredEquipment" = EXCLUDED."RequiredEquipment",
        "RecommendedEquipment" = EXCLUDED."RecommendedEquipment",
  ...
  IF changes ? 'UserDrillAdoption' THEN
    ...
    LOOP
      INSERT INTO "UserDrillAdoption" (
        "UserDrillAdoptionID", "UserID", "DrillID", "Status",
        "HasUnseenUpdate", "IsDeleted", "CreatedAt"
      ) VALUES (
      ...
      ON CONFLICT ("UserDrillAdoptionID") DO UPDATE SET
        "Status" = EXCLUDED."Status",
        "HasUnseenUpdate" = EXCLUDED."HasUnseenUpdate",
        "IsDeleted" = EXCLUDED."IsDeleted";
    END LOOP;
  END IF;
  ...
  IF changes ? 'UserClub' THEN
    ...
    LOOP
      ...
      ON CONFLICT ("ClubID") DO UPDATE SET
        ...
        "Model" = EXCLUDED."Model", "Loft" = EXCLUDED."Loft",
        "Status" = EXCLUDED."Status";
    END LOOP;
  END IF;
  ...
  IF changes ? 'UserTrainingItem' THEN
    ...
    LOOP
      INSERT INTO "UserTrainingItem" (
        "ItemID", "UserID", "Category", "SkillAreas", "Name",
        "Properties", "LinkedClubID", "IsDeleted", "CreatedAt"
      ) VALUES (
        (v_row->>'ItemID')::UUID, v_user_id,
        v_row->>'Category',
        COALESCE(v_row->'SkillAreas', '[]'::JSONB),
        v_row->>'Name',
  ...
  IF changes ? 'Schedule' THEN
    ...
    LOOP
      ...
      ON CONFLICT ("ScheduleID") DO UPDATE SET
        ...
        "Entries" = EXCLUDED."Entries", "Status" = EXCLUDED."Status",
        "IsDeleted" = EXCLUDED."IsDeleted";
    END LOOP;
  END IF;
...
BEGIN
  IF schema_version != '1' THEN
    RETURN jsonb_build_object('success', false, 'error_code', 'SCHEMA_VERSION_MISMATCH',
      'error_message', format('Server expects schema version 1, got %s', schema_version), 'server_timestamp', v_server_ts);
  END IF;

  v_user_id := auth.uid();
  IF v_user_id IS NULL THEN
    RETURN jsonb_build_object('success', false, 'error_code', 'AUTH_REQUIRED',
      'error_message', 'Authentication required for sync', 'server_timestamp', v_server_ts);
  END IF;

  -- === User ===
  SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]'::JSONB) INTO v_rows
  FROM (SELECT "UserID", "DisplayName", "Email", "Timezone", "WeekStartDay", "UnitPreferences", "CreatedAt", "UpdatedAt"
    FROM "User" WHERE "UserID" = v_user_id AND (last_sync_timestamp IS NULL OR "UpdatedAt" > last_sync_timestamp)) t;
  IF v_rows != '[]'::JSONB THEN v_changes := v_changes || jsonb_build_object('User', v_rows); END IF;

  -- === Drill (all columns including Target, Description, units, equipment) ===
  SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]'::JSONB) INTO v_rows
  FROM (
  ...
  -- === PracticeBlock ===
  SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]'::JSONB) INTO v_rows
  FROM (SELECT "PracticeBlockID", "UserID", "SourceRoutineID", "DrillOrder",
           "StartTimestamp", "EndTimestamp", "ClosureType", "EnvironmentType", "SurfaceType",
           "IsDeleted", "CreatedAt", "UpdatedAt"
    FROM "PracticeBlock" WHERE "UserID" = v_user_id
  ...
  -- === Session ===
  SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]'::JSONB) INTO v_rows
  FROM (SELECT s."SessionID", s."DrillID", s."PracticeBlockID", s."CompletionTimestamp",
           s."Status", s."IntegrityFlag", s."IntegritySuppressed",
           s."EnvironmentType", s."SurfaceType",
           s."UserDeclaration", s."SessionDuration", s."IsDeleted", s."CreatedAt", s."UpdatedAt"
  ...
  -- === PracticeEntry ===
  SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]'::JSONB) INTO v_rows
  FROM (SELECT pe."PracticeEntryID", pe."PracticeBlockID", pe."DrillID", pe."SessionID",
           pe."EntryType", pe."PositionIndex", pe."CreatedAt", pe."UpdatedAt"
    FROM "PracticeEntry" pe JOIN "PracticeBlock" pb ON pb."PracticeBlockID" = pe."PracticeBlockID"
  ...
  -- === UserDrillAdoption (includes HasUnseenUpdate) ===
  SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]'::JSONB) INTO v_rows
  FROM (SELECT "UserDrillAdoptionID", "UserID", "DrillID", "Status", "HasUnseenUpdate",
           "IsDeleted", "CreatedAt", "UpdatedAt"
    FROM "UserDrillAdoption" WHERE "UserID" = v_user_id
      AND (last_sync_timestamp IS NULL OR "UpdatedAt" > last_sync_timestamp)) t;
  IF v_rows != '[]'::JSONB THEN v_changes := v_changes || jsonb_build_object('UserDrillAdoption', v_rows); END IF;

  -- === UserClub ===
  SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]'::JSONB) INTO v_rows
  FROM (SELECT "ClubID", "UserID", "ClubType", "Make", "Model", "Loft", "Status", "CreatedAt", "UpdatedAt"
    FROM "UserClub" WHERE "UserID" = v_user_id
      AND (last_sync_timestamp IS NULL OR "UpdatedAt" > last_sync_timestamp)) t;
  IF v_rows != '[]'::JSONB THEN v_changes := v_changes || jsonb_build_object('UserClub', v_rows); END IF;

  -- === UserTrainingItem ===
  SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]'::JSONB) INTO v_rows
  FROM (SELECT "ItemID", "UserID", "Category", "SkillAreas", "Name", "Properties",
           "LinkedClubID", "IsDeleted", "CreatedAt", "UpdatedAt"
    FROM "UserTrainingItem" WHERE "UserID" = v_user_id
      AND (last_sync_timestamp IS NULL OR "UpdatedAt" > last_sync_timestamp)) t;
  IF v_rows != '[]'::JSONB THEN v_changes := v_changes || jsonb_build_object('UserTrainingItem', v_rows); END IF;

  -- === ClubPerformanceProfile ===
  SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]'::JSONB) INTO v_rows
  FROM (SELECT cpp."ProfileID", cpp."ClubID", cpp."EffectiveFromDate",
           cpp."CarryDistance", cpp."DispersionLeft", cpp."DispersionRight",
           cpp."DispersionShort", cpp."DispersionLong", cpp."CreatedAt", cpp."UpdatedAt"
    FROM "ClubPerformanceProfile" cpp JOIN "UserClub" uc ON uc."ClubID" = cpp."ClubID"
    WHERE uc."UserID" = v_user_id AND (last_sync_timestamp IS NULL OR cpp."UpdatedAt" > last_sync_timestamp)) t;
  IF v_rows != '[]'::JSONB THEN v_changes := v_changes || jsonb_build_object('ClubPerformanceProfile', v_rows); END IF;

  -- === UserSkillAreaClubMapping ===
  SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]'::JSONB) INTO v_rows
  FROM (SELECT "MappingID", "UserID", "ClubType", "SkillArea", "IsMandatory", "CreatedAt", "UpdatedAt"
    FROM "UserSkillAreaClubMapping" WHERE "UserID" = v_user_id
      AND (last_sync_timestamp IS NULL OR "UpdatedAt" > last_sync_timestamp)) t;
  IF v_rows != '[]'::JSONB THEN v_changes := v_changes || jsonb_build_object('UserSkillAreaClubMapping', v_rows); END IF;

  -- === Routine ===
  SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]'::JSONB) INTO v_rows
  FROM (SELECT "RoutineID", "UserID", "Name", "Entries", "Status", "IsDeleted", "CreatedAt", "UpdatedAt"
    FROM "Routine" WHERE "UserID" = v_user_id
      AND (last_sync_timestamp IS NULL OR "UpdatedAt" > last_sync_timestamp)) t;
  IF v_rows != '[]'::JSONB THEN v_changes := v_changes || jsonb_build_object('Routine', v_rows); END IF;

  -- === Schedule ===
  SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]'::JSONB) INTO v_rows
  FROM (SELECT "ScheduleID", "UserID", "Name", "ApplicationMode", "Entries", "Status", "IsDeleted", "CreatedAt", "UpdatedAt"
    FROM "Schedule" WHERE "UserID" = v_user_id
      AND (last_sync_timestamp IS NULL OR "UpdatedAt" > last_sync_timestamp)) t;
  IF v_rows != '[]'::JSONB THEN v_changes := v_changes || jsonb_build_object('Schedule', v_rows); END IF;

  -- === CalendarDay ===
  SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]'::JSONB) INTO v_rows
  FROM (SELECT "CalendarDayID", "UserID", "Date", "SlotCapacity", "Slots", "CreatedAt", "UpdatedAt"
    FROM "CalendarDay" WHERE "UserID" = v_user_id
      AND (last_sync_timestamp IS NULL OR "UpdatedAt" > last_sync_timestamp)) t;
  IF v_rows != '[]'::JSONB THEN v_changes := v_changes || jsonb_build_object('CalendarDay', v_rows); END IF;

  -- === RoutineInstance ===
  SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]'::JSONB) INTO v_rows
  FROM (SELECT "RoutineInstanceID", "RoutineID", "UserID", "CalendarDayDate", "OwnedSlots", "CreatedAt", "UpdatedAt"
    FROM "RoutineInstance" WHERE "UserID" = v_user_id
      AND (last_sync_timestamp IS NULL OR "UpdatedAt" > last_sync_timestamp)) t;
  IF v_rows != '[]'::JSONB THEN v_changes := v_changes || jsonb_build_object('RoutineInstance', v_rows); END IF;

  -- === ScheduleInstance ===
  SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]'::JSONB) INTO v_rows
  FROM (SELECT "ScheduleInstanceID", "ScheduleID", "UserID", "StartDate", "EndDate", "OwnedSlots", "CreatedAt", "UpdatedAt"
    FROM "ScheduleInstance" WHERE "UserID" = v_user_id
      AND (last_sync_timestamp IS NULL OR "UpdatedAt" > last_sync_timestamp)) t;
  IF v_rows != '[]'::JSONB THEN v_changes := v_changes || jsonb_build_object('ScheduleInstance', v_rows); END IF;

  -- === EventLog ===
  SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]'::JSONB) INTO v_rows
  FROM (SELECT "EventLogID", "UserID", "DeviceID", "EventTypeID", "Timestamp",
           "AffectedEntityIDs", "AffectedSubskills", "Metadata", "CreatedAt"
    FROM "EventLog" WHERE "UserID" = v_user_id
  ...
  -- === UserDevice ===
  SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]'::JSONB) INTO v_rows
  FROM (SELECT "DeviceID", "UserID", "DeviceLabel", "RegisteredAt", "LastSyncAt", "IsDeleted", "UpdatedAt"
    FROM "UserDevice" WHERE "UserID" = v_user_id
      AND (last_sync_timestamp IS NULL OR "UpdatedAt" > last_sync_timestamp)) t;
  IF v_rows != '[]'::JSONB THEN v_changes := v_changes || jsonb_build_object('UserDevice', v_rows); END IF;

  RETURN jsonb_build_object('success', true, 'server_timestamp', v_server_ts, 'changes', v_changes);

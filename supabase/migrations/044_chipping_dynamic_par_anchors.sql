...
-- Instance strokes stores plus/minus par (positive = over par).
-- MetricSchema par set to 0 since strokes is already relative.
-- Par includes 5% buffer so pro finishes ~2 under par.
-- Anchors (in negated +/- space): Pro = 2 (-2 under), Scratch = -2 (+2 over), Min = -12 (+12 over).
...
UPDATE "Drill"
SET "Anchors" = '{"chipping_distance_control": {"Min": -12, "Scratch": -2, "Pro": 2}, "chipping_direction_control": {"Min": -12, "Scratch": -2, "Pro": 2}}'::JSONB,
    "UpdatedAt" = NOW()

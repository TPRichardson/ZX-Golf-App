-- ============================================================
-- ZX Golf App — Sync Upload RPC
-- Migration: 003_sync_upload.sql
-- TD-03 §5.2.1 — Upload endpoint
-- ============================================================
-- Accepts a JSON payload of changed rows from a client device.
-- Validates schema version, UPSERTs rows per table, enforces
-- Drill structural immutability (TD-03 §5.2.4).
-- Returns {success, server_timestamp, rejected_rows}.
-- ============================================================
  ...
  IF changes ? 'PracticeBlock' THEN
    ...
    LOOP
      INSERT INTO "PracticeBlock" (
        ...
        "StartTimestamp", "EndTimestamp", "ClosureType", "IsDeleted", "CreatedAt"
      ) VALUES (
  ...
  IF changes ? 'Session' THEN
    ...
    LOOP
      INSERT INTO "Session" (
        ...
        "Status", "IntegrityFlag", "IntegritySuppressed", "UserDeclaration",
        "SessionDuration", "IsDeleted", "CreatedAt"
      ) VALUES (

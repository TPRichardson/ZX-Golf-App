-- ============================================================
-- ZX Golf App — Training Kit + Sync Function Patch
-- Migration: 020_training_kit.sql
-- 1. Creates UserTrainingItem table with RLS.
-- 2. Adds RecommendedEquipment column to Drill.
-- 3. Patches sync_upload to include: Drill.RequiredEquipment,
--    Drill.RecommendedEquipment, Drill.Target, Drill.Description,
--    Drill.TargetDistanceUnit, Drill.TargetSizeUnit,
--    UserDrillAdoption.HasUnseenUpdate, UserTrainingItem.
-- 4. Patches sync_download to include same columns + UserTrainingItem.
-- ============================================================

-- === UserTrainingItem table ===
CREATE TABLE IF NOT EXISTS "UserTrainingItem" (
  "ItemID" UUID PRIMARY KEY,
  "UserID" UUID NOT NULL REFERENCES "User"("UserID"),
  "Category" TEXT NOT NULL,
  "SkillAreas" JSONB NOT NULL DEFAULT '[]',
  "Name" TEXT NOT NULL,
  "Properties" JSONB NOT NULL DEFAULT '{}',
  "LinkedClubID" UUID REFERENCES "UserClub"("ClubID"),
  "IsDeleted" BOOLEAN NOT NULL DEFAULT false,
  "CreatedAt" TIMESTAMPTZ NOT NULL DEFAULT now(),
  "UpdatedAt" TIMESTAMPTZ NOT NULL DEFAULT now()
);

CREATE INDEX IF NOT EXISTS idx_user_training_item_user_id ON "UserTrainingItem" ("UserID");

ALTER TABLE "UserTrainingItem" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view own training kit items"
  ON "UserTrainingItem" FOR SELECT
  USING (auth.uid() = "UserID");

CREATE POLICY "Users can insert own training kit items"
  ON "UserTrainingItem" FOR INSERT
  WITH CHECK (auth.uid() = "UserID");

CREATE POLICY "Users can update own training kit items"
  ON "UserTrainingItem" FOR UPDATE
  USING (auth.uid() = "UserID");

-- === Add RecommendedEquipment to Drill ===
ALTER TABLE "Drill" ADD COLUMN IF NOT EXISTS "RecommendedEquipment" JSONB NOT NULL DEFAULT '[]';

-- ============================================================
-- Patch sync_upload — full function replacement
-- Adds: Drill (RequiredEquipment, RecommendedEquipment, Target,
--        Description, TargetDistanceUnit, TargetSizeUnit),
--        UserDrillAdoption (HasUnseenUpdate), UserTrainingItem.
-- ============================================================
...
BEGIN
  ...
  -- === Drill (patched: +Target, Description, TargetDistanceUnit, TargetSizeUnit, RequiredEquipment, RecommendedEquipment) ===
  ...
  -- === UserDrillAdoption (patched: +HasUnseenUpdate) ===
  ...
  IF changes ? 'UserTrainingItem' THEN
    ...
    LOOP
      ...
      ON CONFLICT ("ItemID") DO UPDATE SET
        ...
        "SkillAreas" = EXCLUDED."SkillAreas",
        "Name" = EXCLUDED."Name",
        "Properties" = EXCLUDED."Properties",
        "LinkedClubID" = EXCLUDED."LinkedClubID",
        "IsDeleted" = EXCLUDED."IsDeleted";
    END LOOP;
  END IF;
  ...
  IF changes ? 'CalendarDay' THEN
    ...
    LOOP
      ...
      ON CONFLICT ("CalendarDayID") DO UPDATE SET
        "Date" = EXCLUDED."Date", "SlotCapacity" = EXCLUDED."SlotCapacity",
  ...
  IF changes ? 'RoutineInstance' THEN
    ...
    LOOP
      INSERT INTO "RoutineInstance" (
        "RoutineInstanceID", "RoutineID", "UserID", "CalendarDayDate", "OwnedSlots", "CreatedAt"
      ) VALUES (
  ...
  IF changes ? 'EventLog' THEN
    ...
    LOOP
      INSERT INTO "EventLog" (
        ...
        (v_row->>'EventLogID')::UUID, v_user_id, v_row->>'DeviceID', v_row->>'EventTypeID',
  ...
  IF changes ? 'UserDevice' THEN
    ...
    LOOP
      INSERT INTO "UserDevice" (
        "DeviceID", "UserID", "DeviceLabel", "RegisteredAt", "LastSyncAt", "IsDeleted"
      ) VALUES (
...
-- Adds: Drill (Target, Description, TargetDistanceUnit, TargetSizeUnit,
--        RequiredEquipment, RecommendedEquipment),
--        UserDrillAdoption (HasUnseenUpdate), UserTrainingItem.
-- ============================================================
...
BEGIN
  ...
  FROM (
    SELECT s."SessionID", s."DrillID", s."PracticeBlockID",
           ...
           s."IntegritySuppressed", s."EnvironmentType", s."SurfaceType",
  ...
  FROM (
    SELECT "UserDrillAdoptionID", "UserID", "DrillID", "Status",
           "HasUnseenUpdate", "IsDeleted", "CreatedAt", "UpdatedAt"
  ...
  SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]'::JSONB) INTO v_rows
  FROM (
    SELECT "ItemID", "UserID", "Category", "SkillAreas", "Name",
           "Properties", "LinkedClubID", "IsDeleted", "CreatedAt", "UpdatedAt"
    FROM "UserTrainingItem"
    WHERE "UserID" = v_user_id
      AND (last_sync_timestamp IS NULL OR "UpdatedAt" > last_sync_timestamp)
  ) t;
  ...
  -- === EventLog (uses CreatedAt — append-only) ===
  SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]'::JSONB) INTO v_rows
  FROM (
